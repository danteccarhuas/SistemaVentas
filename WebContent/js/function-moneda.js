$(document)	.ready(function(e) {
	...
	$('#btn_salir').on('click', function () {
		$("#frm_moneda").data('bootstrapValidator').resetForm(true);/*Limpiamos todos los controles del formulario frm_Proveedor */
	...
	$(document).on("click","#btn_modaleliminar",function(e) {
		e.preventDefault();		
		$('#hiddencodmoneda').val($(this).data('id'));
		$("#modalRemove").modal({
			keyboard : false
		});
	});
	...
	$(".removeBtn").click(function(e){
		...
		console.log($('#hiddencodmoneda').val());
		var  codmoneda=$('#hiddencodmoneda').val();
		$.ajax({
			url : 'moneda?metodo=EliminarMoneda',
			type : 'post',
	...
	$(document).on("click","#btn_editar",function(e) {			
		...
		var codigo_moneda = $('#hiddencodmoneda').val();		
		$.ajax({
			url : 'moneda?metodo=ObtenerMoneda',
			type : 'post',
			data : {codigo_moneda:codigo_moneda},
			dataType : 'json',
			success : function(result) {
				var data1=result[0];
				...
				$('#eventotab2primary').prop( "disabled", false ).removeClass('disabled');/*Quitamos el disabled del tab eventotab2primary*/
				$('.nav-tabs > .active').prop( "disabled", true ).addClass('disabled').next('li').find('a').trigger('click');			
				$("#tab2primary #txt_codigo_guardar").val(data1.idmoneda);
				$("#tab2primary #txt_descripcion_guardar").val(data1.descripcion);		
				$("#tab2primary #txt_simbolo_guardar").val(data1.simbolo);		
			}
		});
	});
	/* Valida las etiquedas del formulario */
	$("#frm_moneda").bootstrapValidator({
			...
			fields : {
				...
				"txt_simbolo_guardar" : {
					validators : {
						notEmpty : {
							message : 'Ingrese simbolo por favor.'
						}
					}
				}
			}
	});
	$('#frm_moneda').on('success.form.bv', function(e) {
		e.preventDefault();
		GuardarMoneda();
	...
	function GuardarMoneda(){		 
		  $('#ModalLoading').modal('show');
		  $.ajax({
				url : 'moneda?metodo=RegistrarModificarMoneda',
				type : 'post',
				data : $('#frm_moneda').serialize(),
				dataType:'json',
				success:function(result){
					$('#ModalLoading').modal('hide');
					...
					if(valor.indAccion=="1"){
						if(valor.codigomoneda=="-1"){
							$('#mensajeAlerta').html("<div class='alert alert-warning'>Ocurrio un error al registrar los datos de la moneda</div>");
						}else{
							$("#tab2primary #txt_codigo_guardar").val(valor.codigomoneda);
							$('#mensajeAlerta').html("<div class='alert alert-success'>Se registro satisfactoriamente los datos de la moneda con el codigo : "+ valor.codigomoneda +"</div>");
						}
					}else{
						if(valor.codigomoneda=="-1"){
							$('#mensajeAlerta').html("<div class='alert alert-warning'>Ocurrio un error al modificar los datos de la moneda</div>");
						}else{							
							$('#mensajeAlerta').html("<div class='alert alert-success'>Se modificaron satisfactoriamente los datos de la moneda</div>");
						}
					}
				},
...
function cargaPagina(pagina){
	...
	$.ajax({
		url : 'moneda?metodo=ListarMoneda',
		type : 'post',
			...
			if (lista.length > 0){
				for ( var i = 0; i < lista.length; i++) {
					trHTML += '<tr><td>'
						...
						+ '</td><td>'
						+ lista[i]['simbolo']
						+ '</td><td><a href="" data-id="'+lista[i]['idmoneda']+'" id="btn_editar" class="btn btn-info"><span class="fa fa-pencil-square-o " ></span> </a></td>'
						+ '</td><td><a  data-id="'+lista[i]['idmoneda']+'" id="btn_modaleliminar" class="btn btn-danger"><span class="fa fa-trash-o" ></span></a></td>';
				}
...
function initGrilla(){
	...
	var txt_Descripcion_buscar = $("#txt_Descripcion_buscar").val();
	$.ajax({
		url : 'moneda?metodo=TotalRegistrosMoneda',
		type : 'post',
